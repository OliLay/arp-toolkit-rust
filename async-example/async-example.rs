...
async fn start() {
    ...
    futures::join!(resolve_simple(mac_addr, ip_addr), resolve_advanced(mac_addr, ip_addr));
}
...
async fn resolve_simple(mac_addr: MacAddr, ip_addr: Ipv4Addr) {
    ...
    println!("Simple: IP for MAC {} is {}", mac_addr, result.await.unwrap());

    let result = client.ip_to_mac(ip_addr, None);
    println!("Simple: MAC for IP {} is {}", ip_addr, result.await.unwrap());
}
...
async fn resolve_advanced(mac_addr: MacAddr, ip_addr: Ipv4Addr) {
    ...
    let result = client.send_request(None, arp_request).await.unwrap();
    println!(
    ...
    let result = client.send_request(None, rarp_request).await.unwrap();
    println!(
